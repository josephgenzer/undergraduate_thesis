...
simulate_Y <- function(n, H1, H2, P, p) {
...
simulate_eigenvalues <- function(r) {
  Y <- simulate_Y(n, H1, H2, P, p)
  M_h <- calculate_M(Y, h)
  lambda <- sort(eigen(M_h, symmetric = TRUE)$values, decreasing = TRUE)
  transformed <- sqrt(n / h) * c(log(lambda[1]) - centering2,
                                 log(lambda[2]) - centering1)
  return(transformed)
}
...
eigen_df$norm_lambda_p_minus1 <- as.numeric(scale(eigen_df$lambda_p_minus1))
eigen_df$norm_lambda_p <- as.numeric(scale(eigen_df$lambda_p))
...
Mh_diffs <- eigen_df$lambda_p - eigen_df$lambda_p_minus1
